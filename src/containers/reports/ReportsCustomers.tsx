...
export const ReportsCustomers = observer(() => {
    const history = useHistory()
    const [reportsStore] = useState(() => new ReportStore())

    useEffect(() => {
        reportsStore.loadCustomers()
    }, [])

    return (
        <View style={styles.container}>
            <Text style={styles.heading}>Report Customers Totals</Text>
            {reportsStore.isLoading && <Text>Loading data...</Text>}
            {!reportsStore.customers?.length && !reportsStore.isLoading && <Text>Empty list</Text>}
            <FlatList
                data={reportsStore.customers}
