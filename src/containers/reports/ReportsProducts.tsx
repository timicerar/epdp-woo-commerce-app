...
export const ReportsProducts = observer(() => {
    const history = useHistory()
    const [reportsStore] = useState(() => new ReportStore())

    useEffect(() => {
        reportsStore.loadProducts()
    }, [])

    return (
        <View style={styles.container}>
            <Text style={styles.heading}>Report Products Totals</Text>
            {reportsStore.isLoading && <Text>Loading data...</Text>}
            {!reportsStore.products?.length && !reportsStore.isLoading && <Text>Empty list</Text>}
            <FlatList
                data={reportsStore.products}
