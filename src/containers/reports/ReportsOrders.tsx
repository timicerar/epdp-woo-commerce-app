...
export const ReportsOrders = observer(() => {
    const history = useHistory()
    const [reportsStore] = useState(() => new ReportStore())

    useEffect(() => {
        reportsStore.loadOrders()
    }, [])

    return (
        <View style={styles.container}>
            <Text style={styles.heading}>Report Orders Totals</Text>
            {reportsStore.isLoading && <Text>Loading data...</Text>}
            {!reportsStore.orders?.length && !reportsStore.isLoading && <Text>Empty list</Text>}
            <FlatList
                data={reportsStore.orders}
