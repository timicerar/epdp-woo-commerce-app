...
export const ReportsCoupons = observer(() => {
    const history = useHistory()
    const [reportsStore] = useState(() => new ReportStore())

    useEffect(() => {
        reportsStore.loadCoupons()
    }, [])

    return (
        <View style={styles.container}>
            <Text style={styles.heading}>Report Coupons Totals</Text>
            {reportsStore.isLoading && <Text>Loading data...</Text>}
            {!reportsStore.coupons?.length && !reportsStore.isLoading && <Text>Empty list</Text>}
            <FlatList
                data={reportsStore.coupons}
